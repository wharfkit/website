...
The `SessionKit` class provided by the [Session Kit](/docs/sessionkit) is responsible for creating and managing end user [Session](#) instances specifically for **web applications** through actions such as [login](#) and [restore](#). Only **one instance is required** for the entire application and should be made widely available.
