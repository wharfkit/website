...
This document contains Revision 1 of the Resource Provider Specification. This specification can be adopted and applied by any Antelope blockchain which has enabled the [only bill first authorizer](/docs/utilities/resource-provider-spec#only-billing-the-first-authorizer) feature enabled, and is natively supported by Wharf's [Session Kit](/docs/session-kit) through the use of optional plugins.
...
The Resource Provider specification defines set processes which can be deployed to abstract away these complexities, either by providing resources for free, or in exchange for a simple fee. This greatly improves the end user experience, while still allowing advanced users to utilize the more complex native resource management systems.
...
- A resource provider, running an API endpoint that's willing to service requests.
- An end user's wallet/signer, which will authorize the transactions for the user.
...
The standard is designed to work with any combination of supported SDK and service provider, and is optionally enhanced by adoption of the user's wallet/signer. The SDKs can gracefully fail continuing on with the transaction if the service provider fails to respond. The blockchain itself requires no support or knowledge of these processes in order to receive the transaction.

## Specification

The design of a resource provider API consists of a single endpoint, which will respond with one of three types of responses. An application or its SDKs will understand how to perform these requests, a service provider will respond to these requests, and an end user's wallet/signer will optionally understand how to best display the metadata around this request.
...
A [PackedTransaction](/docs/antelope/transaction#packedtransaction) may be passed as the `packedTransaction` value along with the `signer`. The `signatures` array within the packed transaction will be empty, since the resource provider will provide the first signature.
...
The resource provider API endpoint will respond with one of three potential response types, based on business logic it internally defines. Each of these responses has different HTTP response status codes, mirrored as both the `code` value in the JSON as well as in the API response headers.
...
- A [noop action](/docs/utilities/resource-provider-spec#the-noop-action) was prepended to the actions array to make the resource provider's account the [first authorizer](/docs/utilities/resource-provider-spec#only-billing-the-first-authorizer), to cover both CPU and NET resource costs of the transaction.
...
- A RAM purchase action was appended to the transaction to purchase additional RAM resources from the network that will be allocated to the user's account.
...
The Resource Provider specification offers use cases for a number of different entities that may provide network resources to users of specific applications and/or blockchains.
...
The methods defined in this specification have broad potential beyond the use cases defined above, and may be implemented by any party when resource management becomes a barrier to adoption.

## Implementations

The Resource Provider model has now been in use since 2020, utilizing this Revision 1 specification. As this list continues to grow, pull requests to this documentation are welcome to help expand and illustrate its usage throughout the Antelope ecosystem.
...
This plugin can be added to any project using Wharf by running:
...
This plugin can be included in any application utilizing Wharf, and configured to access a specific resource provider by providing its URL. This resource provider plugin defaults to the Fuel API endpoints provided by [Greymass](https://www.greymass.com/products).

### Resource Providers

A number of deployments following this standard exist within the Antelope ecosystem, many of which are used only for specific applications, and some of which are general-purpose for any application to make use of.
...
While any wallet/signer can sign these types of transactions, wallets that implement the metadata from the resource providers can display this data to end users to help inform them of the fees being paid and improve their user experience.
...
An example resource provider service implementation for Revision 1 of the resource provider specification was released to illustrate how one might build a service to provide these types of resources.
...
From a wallet/signer perspective, these transactions are received like any other transaction that requires a signature. Additional metadata may optionally be passed from the application to the signer to improve UX, but this data is not required for the resource provider model to function.
...
The Resource Provider specification utilizes this feature to allow one specific account to cover the resource costs of the entire transaction, regardless of the other parties involved in the other actions. This is done by prepending a [noop action](/docs/utilities/resource-provider-spec#the-noop-action) into the actions array of the transaction and providing a signature by the party assuming the cost of the resources.
