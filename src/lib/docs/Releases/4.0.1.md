...
A page that loads `@wharfkit/bundle` from a CDN upgrades by changing its script tag. Load the file by exact version and full path, which is the only form that serves the bytes npm published:

```html
<script src="https://cdn.jsdelivr.net/npm/@wharfkit/bundle@4.0.1/dist/wharf.bundle.js"></script>
```
...
- `PublicKey.from()` parses legacy `EOS` keys shortened by leading zero bytes, such as the null key `EOS1111111111111111111111111111111114T1Anm`.
...
- `WalletPlugin` has an optional `clone()`, implemented on `AbstractWalletPlugin`, which `SessionKit` calls to give each restored session its own instance. `SessionKit.getWalletPlugin(id)` returns a registered plugin by its id.
- `@wharfkit/roborovski` gains `activity()`, which pages through an account's history with `next()` and `prev()` and filters by contract, action and date range, and `transaction()` reads a transaction by id through the same route.
- `@wharfkit/web-renderer` accepts `colorMode`, `"light"` or `"dark"`: `new WebRenderer({ colorMode: "dark" })`.
...
- `@wharfkit/account-creation-plugin-anchor` works with a `serviceUrl` that has a path, and a `serviceUrl` passed in configuration reaches the account creator.
