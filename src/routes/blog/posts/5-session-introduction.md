...
> Before continuing on, it's important to note that this article contains a "Technical Preview" using an early release of Wharf's [Session Kit](https://github.com/wharfkit/session). This article was written in early 2023 using `@wharfkit/session` version 0.1.1. Everything within this article is subject to change as the code matures. Please refer to the official documentation (when available) on [WharfKit.com](https://wharfkit.com) for up-to-date usage instructions and information.

[Wharf](https://wharfkit.com) is a collection of Software Development Kits (SDKs) for developers working on [Antelope](https://antelope.io) applications using JavaScript. This article gives a preview of the upcoming release of the [Session Kit](https://github.com/wharfkit/session), one of the major components in the Wharf product collection. The Session Kit aims to provide the tools required for developers to interact with an Antelope-based blockchain.

## Session Kit

With the introduction of the [Session Kit](https://github.com/wharfkit/session), the goal is to change the developer experience to something closer to what they'd expect from other web technologies. A "session" is a technical concept in development that represents the connection between an authenticated device and a remote application server. When a user authenticates against an application server, a session is established which allows further actions to be performed by a specific device.

During the development of Wharf, it was decided to adopt the concept of sessions and apply it to interactions with Antelope-based blockchains. This provides new developers with familiarity to a system they already know in order to ease the adoption curve.
...
The Session Kit at the time of this writing is made up of 3 major components, each representing a different aspect of Antelope-based application development.
...
Developers working with the Session Kit will primarily be working with an individual `Session`, whether crafted by hand within an application or by the `SessionKit` factory methods. A session represents the connection between a device and account on an Antelope-based blockchain. These sessions offer a `transact` method that allows actions to be performed on the blockchain by the account it is associated with.
...
- The `SessionKit` can optionally utilize `LoginPlugins`, which can modify the logic of the factory during the `login` call.
- Each `Session` can optionally utilize `TransactPlugins`, which can modify the logic of the `Session` during the `transact` call.
...
So far any developer experienced in application development on Antelope-based blockchains will be very well versed with the design paradigms of the Session Kit. To shift focus and now highlight the additional benefit that the Session Kit brings, the focus must shift to the Plugin architecture.

This feature helps differentiate it from previous solutions by providing additional flexibility and customization. Plugins offer an opportunity for developers to hook into many of the predefined processes within the Session and extend its capabilities. The Session makes use of the [TransactPlugin](https://wharfkit.github.io/session/interfaces/TransactPlugin.html) type. A TransactPlugin can register custom logic at 4 different points in the `transact` flow before & after the sign & broadcast phases.

In this example the script is now going to utilize a Resource Provider plugin, one which analyzes a transaction to determine if the account requires network resources to perform. The source code for this plugin can be found here:

https://github.com/wharfkit/resource-provider-plugin

Using the same code example as above, to include a plugin the code just needs a slight modification. As of version 0.1.1 of the `@wharfkit/session` library, the plugin would be passed in during the creation of the `Session` as follows:
...
Under the hood the `ResourceProviderPlugin` is analyzing the transaction itself before its signed (using the `beforeSign` transaction hook) to ensure the account has the appropriate network resources to successfully execute. If it does not, it has the potential to modify the transaction to include additional actions to ensure it succeeds.
...
The Session Kit is available now as an early release and available on [npmjs.com](https://www.npmjs.com/package/@wharfkit/session). If anyone would like to try it out in a backend project today, simply include it in your project using either `npm` or `yarn`.
...
Its capabilities at the time of this writing are very limited and are subject to radical changes, so it is advised to only use for development and experimentation purposes.

Over the coming months the Session Kit will continue to evolve based on our current plans and feedback collected from the developer community. It is anticipated that the following additional features will be added next:
...
While the Session Kit is still not ready for production applications, hopefully the benefits outlined within this article show the benefits it already provides and how as time passes it will continue to empower developers building on Antelope-based blockchains.

## Feedback and Collaboration

Feedback and collaboration from the developer community is always welcome. If you’d like to adopt and start using the Session Kit early on in order to help guide its development, give it a try.

To communicate with Greymass and other developers involved about this project, listed below are a number of platforms being used:

- [The WharfKit.com website contains recorded calls and (eventually) documentation](https://wharfkit.com).
